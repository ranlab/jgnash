...
public abstract class BaseDynamicJasperReport {
    ...
    public final void setBaseFontSize(final int size) {
        Preferences p = getPreferences();
    ...
    protected final JasperPrint createJasperPrint(final AbstractReportTableModel model, final boolean formatForCSV) {
        ...
        if (!formatForCSV) {
            ...
            final AutoText pageNum = new AutoText(AutoText.AUTOTEXT_PAGE_X_SLASH_Y, AutoText.POSITION_FOOTER,
                    HorizontalBandAlignment.RIGHT, (byte) 0, 80, 50);
